import 'bloc/single_batch_income_two_bloc.dart';
import 'models/single_batch_income_two_model.dart';
...
class SingleBatchIncomeTwoScreen extends StatelessWidget {
  SingleBatchIncomeTwoScreen({Key? key})
      : super(
          key: key,
        );
  ...
  static Widget builder(BuildContext context) {
    return BlocProvider<SingleBatchIncomeTwoBloc>(
      create: (context) => SingleBatchIncomeTwoBloc(SingleBatchIncomeTwoState(
        singleBatchIncomeTwoModelObj: SingleBatchIncomeTwoModel(),
      ))
        ..add(SingleBatchIncomeTwoInitialEvent()),
      child: SingleBatchIncomeTwoScreen(),
    );
  }
  ...
  Widget _buildAddIncomeName(BuildContext context) {
    return BlocSelector<SingleBatchIncomeTwoBloc, SingleBatchIncomeTwoState,
  ...
  Widget _buildAddIncomeCategoryValue(BuildContext context) {
    return Expanded(
      child: BlocSelector<SingleBatchIncomeTwoBloc, SingleBatchIncomeTwoState,
          TextEditingController?>(
  ...
  Widget _buildAddIncomePaymentMethod(BuildContext context) {
    return BlocBuilder<SingleBatchIncomeTwoBloc, SingleBatchIncomeTwoState>(
      builder: (context, state) {
        return state.singleBatchIncomeTwoModelObj!.radioList.isNotEmpty
            ? Row(
                children: [
                  CustomRadioButton(
                    text: "lbl_cash".tr,
                    value:
                        state.singleBatchIncomeTwoModelObj?.radioList[0] ?? "",
                    groupValue: state.addIncomePaymentMethod,
                    padding: EdgeInsets.symmetric(vertical: 1.v),
                    onChange: (value) {
                      context
                          .read<SingleBatchIncomeTwoBloc>()
                  ...
                  Padding(
                    ...
                    child: CustomRadioButton(
                      text: "lbl_online".tr,
                      value: state.singleBatchIncomeTwoModelObj?.radioList[1] ??
                          "",
                      groupValue: state.addIncomePaymentMethod,
                      padding: EdgeInsets.symmetric(vertical: 1.v),
                      onChange: (value) {
                  ...
                  Padding(
                    ...
                    child: CustomRadioButton(
                      text: "lbl_unpaid".tr,
                      value: state.singleBatchIncomeTwoModelObj?.radioList[2] ??
                          "",
                      groupValue: state.addIncomePaymentMethod,
                      padding: EdgeInsets.symmetric(vertical: 1.v),
                      onChange: (value) {
