import 'bloc/single_batch_income_one_bloc.dart';
import 'models/single_batch_income_one_model.dart';
...
class SingleBatchIncomeOneScreen extends StatelessWidget {
  SingleBatchIncomeOneScreen({Key? key})
      : super(
          key: key,
        );
  ...
  static Widget builder(BuildContext context) {
    return BlocProvider<SingleBatchIncomeOneBloc>(
      create: (context) => SingleBatchIncomeOneBloc(SingleBatchIncomeOneState(
        singleBatchIncomeOneModelObj: SingleBatchIncomeOneModel(),
      ))
        ..add(SingleBatchIncomeOneInitialEvent()),
      child: SingleBatchIncomeOneScreen(),
    );
  }
  ...
  Widget _buildAddIncomeName(BuildContext context) {
    return BlocSelector<SingleBatchIncomeOneBloc, SingleBatchIncomeOneState,
  ...
  Widget _buildAddIncomeCategoryValue(BuildContext context) {
    return Expanded(
      child: BlocSelector<SingleBatchIncomeOneBloc, SingleBatchIncomeOneState,
          TextEditingController?>(
  ...
  Widget _buildAddIncomePaymentMethod(BuildContext context) {
    return BlocBuilder<SingleBatchIncomeOneBloc, SingleBatchIncomeOneState>(
      builder: (context, state) {
        return state.singleBatchIncomeOneModelObj!.radioList.isNotEmpty
            ? Row(
                children: [
                  CustomRadioButton(
                    text: "lbl_cash".tr,
                    value:
                        state.singleBatchIncomeOneModelObj?.radioList[0] ?? "",
                    groupValue: state.addIncomePaymentMethod,
                    padding: EdgeInsets.symmetric(vertical: 1.v),
                    onChange: (value) {
                      context
                          .read<SingleBatchIncomeOneBloc>()
                  ...
                  Padding(
                    ...
                    child: CustomRadioButton(
                      text: "lbl_online".tr,
                      value: state.singleBatchIncomeOneModelObj?.radioList[1] ??
                          "",
                      groupValue: state.addIncomePaymentMethod,
                      padding: EdgeInsets.symmetric(vertical: 1.v),
                      onChange: (value) {
                  ...
                  Padding(
                    ...
                    child: CustomRadioButton(
                      text: "lbl_unpaid".tr,
                      value: state.singleBatchIncomeOneModelObj?.radioList[2] ??
                          "",
                      groupValue: state.addIncomePaymentMethod,
                      padding: EdgeInsets.symmetric(vertical: 1.v),
                      onChange: (value) {
